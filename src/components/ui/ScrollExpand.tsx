...
export default function ScrollExpand({
  ...
  return (
    <div
      ref={rootRef}
      className={`relative h-full w-full ${useWindowScroll ? "" : "overflow-x-hidden overflow-y-auto overscroll-contain [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden"} ${className}`.trim()}
      style={style}
    >
      <div ref={trackRef} className="relative w-full">
        ...
        >
          ...
          {title ? (
            <div
              ref={titleRef}
              className="pointer-events-none absolute inset-0 m-0 flex items-center justify-center px-[6%] text-center font-display font-bold leading-none tracking-[-0.03em] text-white [font-size:var(--se-title-size)] [text-shadow:0_2px_24px_rgba(0,0,0,0.45)] [will-change:opacity,transform]"
            >
              {title}
            </div>
          ) : null}
